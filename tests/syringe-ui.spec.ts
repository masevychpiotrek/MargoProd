...
test('start, two entries, downtime, handover and next shift from zero', async ({ page }, testInfo) => {
  ...
  await page.route('**/*', async route => {
    ...
    try {
      ...
      for (const [key, filter] of url.searchParams) {
        ...
        else if (op === 'gte') rows = rows.filter(r => (read(r) instanceof Date ? read(r).toISOString() : String(read(r))) >= value)
        else if (op === 'lte') rows = rows.filter(r => (read(r) instanceof Date ? read(r).toISOString() : String(read(r))) <= value)
  ...
  try {
    ...
    await expect(page.getByRole('button', { name: /^Strzykawka (5|10|20|50|100) ml/ })).toHaveCount(0)
    await expect(page.getByRole('button', { name: /^Strzykawka 2 ml · Standard/ })).toBeVisible()
    await page.screenshot({ path: testInfo.outputPath('line-2ml-variants.png'), fullPage: true })
    await page.getByRole('button', { name: /^Strzykawka 2 ml · Nominał/ }).click()
    ...
    await page.getByRole('button', { name: /Wpisz produkcję Stan licznika/ }).click()
    ...
    await page.getByRole('button', { name: 'Przypisz pozostałe +50', exact: true }).click()
    rejectNext = true
    await page.getByRole('button', { name: 'Zapisz produkcję' }).click()
    ...
    await expect(page.getByRole('heading', { name: 'Linia testowa 1' })).toBeVisible()
    await page.getByRole('button', { name: /Zamknij zmianę/ }).click()
    await expect(page.getByText(/Liczniki końcowe zostaną pobrane automatycznie/)).toBeVisible()
    await page.getByPlaceholder(/Napisz dlaczego zamykasz zmianę/).fill('Zakończenie testowej zmiany operatora')
    const earlyConfirm = page.getByRole('checkbox')
    if (await earlyConfirm.count()) await earlyConfirm.check()
    ...
    await go('/changeover')
    await page.getByRole('button', { name: /^Strzykawka 2 ml · Standard/ }).click()
    await expect(page.getByRole('button', { name: /^Strzykawka 5 ml/ })).toHaveCount(0)
    await page.getByPlaceholder('Stan druku przed', { exact: true }).fill('100')
    ...
    await go('/entry')
    await page.getByLabel('Licznik druku', { exact: true }).fill('200')
    await page.getByLabel('Licznik montażu', { exact: true }).fill('200')
    await page.getByRole('button', { name: 'Zapisz produkcję' }).click()
    await expect(page.getByRole('heading', { name: 'Linia testowa 1' })).toBeVisible()
    expect((await db.query('SELECT total_good FROM sa_sessions WHERE ended_at IS NULL')).rows[0].total_good).toBe(200)
    await go('/quality')
